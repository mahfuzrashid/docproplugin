...
if ( wp_verify_nonce( docpro()->get_args_option( 'docpro_profile_val', '', $posted_data ), 'docpro_profile' ) ) {
	...
	$data_args   = array(
		'first_name' => docpro()->get_args_option( 'first_name', $doctor->first_name, $posted_data ),
		'last_name'  => docpro()->get_args_option( 'last_name', $doctor->last_name, $posted_data ),
	);
	$meta_fields = docpro()->metaBoxes->get_meta_fields( 'doctor' );
	...
	if ( $ret = $doctor->update_data( $data_args ) ) {
...
<form action="<?php echo esc_url( site_url( $wp->request ) ); ?>" class="add-listing my-profile" method="post">
	...
	<?php docpro()->metaBoxes->render_frontend_user_form_fields( $doctor->ID ); ?>

...
if ( wp_verify_nonce( docpro()->get_args_option( 'docpro_pw_nonce_val', '', $posted_data ), 'docpro_pw_nonce' ) ) {
	...
	if ( wp_check_password( $old_password, $doctor->user_pass, $doctor->ID ) && $new_password == $confirm_password ) {
		wp_set_password( $new_password, $doctor->ID );

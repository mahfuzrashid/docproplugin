...
if ( ! class_exists( 'CSF' ) ) {
	class CSF {
		...
		public static function init() {
			...
			add_action( 'init', array( 'CSF', 'setup' ), 99 );
			...
			if ( is_admin() ) {
				...
				if ( ! empty( self::$shortcode_instances ) ) {
					foreach ( self::$shortcode_instances as $argument ) {
						if ( ( $argument['show_in_editor'] && $wpscreen && $wpscreen->base === 'post' ) || $argument['show_in_custom'] ) {
							$enqueue = true;
						}
					}
				}
				...
				if ( $wpscreen && $wpscreen->id === 'tools_page_csf-welcome' ) {
					$enqueue = true;
				}
			}
